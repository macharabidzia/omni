...
if (!url || !fakeAudioPath || !outputPath) {
  console.error("usage: node playwright-livekit-check.mjs <url> <fake-audio.wav> <output.json>");
  process.exit(2);
}
  ...
  try {
    ...
    try {
      await page.waitForFunction(
        () => {
          const debugText = document.querySelector(".debug-log")?.textContent ?? "";
          return debugText.includes('"type": "assistant.done"');
        },
        undefined,
        { timeout: 5000 },
      );
    } catch {
      // Best effort: short replies usually finish quickly, but keep the probe bounded.
    }
